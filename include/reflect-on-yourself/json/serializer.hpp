...
namespace roy::json::detail{
	template<typename TSource> requires(with_type_info<TSource> && roy::type_info_for_t<TSource>::fields::size() > 0)
	...
	template<typename TField> requires(field_type<TField>)
	inline void serialize(std::ostream& stream, const typename TField::type& obj);

	template<typename TTypeInfo, std::size_t... Is> requires(type_info_type<TTypeInfo>)
	inline void serialize(std::ostream& stream, const typename TTypeInfo::type& obj, std::index_sequence<Is...>);

	template<typename TTypeInfo> requires(type_info_type<TTypeInfo>)
	...
	inline void serialize(std::ostream& stream, const typename TField::type& obj){
		...
		if constexpr(field_type_info::fields::size() == 0){
			...
			stream << '"' << TField::name << '"' << ':' << '{';
			detail::serialize<field_type_info>(stream, obj, std::make_index_sequence<field_type_info::fields::size()>{});
			stream << '}';
		}
	}
